...
import { MessageSquare, X, Send, Trash2, ChevronLeft, Search, Smile } from 'lucide-react';
import { useMessaging } from '../contexts/MessagingContext';
import { useAuth } from '../hooks/useAuth';
import type { ConversationParticipant } from '../types/messaging';
...
const STYLES = `
  ...
  .msg-conv-delete {
    ...
    color: #D0CBC3; border-radius: 8px;
    opacity: 0; transition: opacity 0.15s, color 0.15s;
    display: flex; align-items: center; justify-content: center;
  }
  .msg-conv-item:hover .msg-conv-delete { opacity: 1; }
  .msg-conv-delete:hover { color: #EF4444; }
  ...
  .msg-back-btn {
    background: #F4F3F0; border: none; border-radius: 12px;
  ...
  .msg-chat-name {
    font-size: 15px; font-weight: 700; color: #18181B; letter-spacing: -0.3px;
  }
  .msg-chat-role {
    font-size: 12px; color: #FF7A35; font-weight: 600; text-transform: uppercase;
  ...
  .msg-bubble-text { font-size: 14px; line-height: 1.5; margin: 0; }
  .msg-bubble-time {
    font-size: 10.5px; margin-top: 4px; display: block;
  }
  ...
  .msg-placeholder h3 { margin:0; font-size:17px; font-weight:700; color:#7A746C; letter-spacing:-0.3px; }
...
const MessagingUI: React.FC<{
  ...
  const filtered = conversations.filter(c => {
    ...
    const other = ids.find(id => id !== currentUser?.uid) || ids[0];
    return c.participants[other]?.name?.toLowerCase().includes(search.toLowerCase());
  });
...
const ConversationItem: React.FC<{
  ...
  onClick: () => void;
  onDelete: () => void;
}> = ({ conversation, isActive, currentUserId, onClick, onDelete }) => {
